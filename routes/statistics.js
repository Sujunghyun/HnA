...
router.get('/prof_statistic/:l_id/:u_role', (req, res) => {
  ...
  const l_id = req.params.l_id, u_role = req.params.u_role,
    date_sql = 'SELECT l_date from LECTURE_DT where l_id = ? order by l_date;',
    count_sql = 'SELECT ' +
      ...
      async.waterfall(tasks, function (err) {
        if (err)
          ...
          for (let k = 0; k < stu_list.length; k++) {       // 출석 데이터를 전부 수강데이터에 적용.
              ...
              attend_list[k][i].u_name = stu_list[k][i].u_name;
              attend_list[k][i].photo_url = stu_list[k][i].photo_url;
						}
					}
          // prof_stats
          for (let i = 0; i < prof_stats.length; i++) {
            prof_stats[i].s_list = attend_list[i];
          }
          console.log('done');
    ...
    if (u_role == 'stu') {
      ...
      async.waterfall(tasks, function (err) {
        if (err)
          ...
          statistics = Math.floor(stats_temp * 100) + '%';
